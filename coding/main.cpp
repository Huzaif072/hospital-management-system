...
int main() {
  ...
  sf::RenderWindow window(sf::VideoMode::getDesktopMode(),
                          "Hospital Management System", sf::Style::Fullscreen);
  ...
  menuBackgroundSprite.setScale(static_cast<float>(window.getSize().x) /
  ...
  if (!loadingBackgroundTexture.loadFromFile(
          "img/Loading_Screen.jpg")) { // Use your loading screen background
                                       // image
    std::cerr << "Error: Failed to load loading screen background image"
              << std::endl;
    return -1;
  }
  loadingBackgroundTexture.setSmooth(true);
  sf::Sprite loadingBackgroundSprite(loadingBackgroundTexture);
  loadingBackgroundSprite.setScale(static_cast<float>(window.getSize().x) /
                                       loadingBackgroundTexture.getSize().x,
                                   static_cast<float>(window.getSize().y) /
                                       loadingBackgroundTexture.getSize().y);
  ...
  LoadingScreen::show(window, loadingBackgroundSprite, font,
                      sf::Color::Black); // Use your custom loading background

  // Initialize user credentials
  std::unordered_map<std::string, std::string> users = {
      {"huzaif", "huzaif1234"}, {"admin", "admin1234"}};
  ...
  // Initialize patient list and menu, pass the window reference to the menu
  // constructor
